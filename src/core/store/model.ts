 ...
 */
export type Surface = 'chrome' | 'firefox' | 'sovereign' | 'companion' | 'mobile';
...
export interface JournalEntry {
  ...
  surface: Surface;
  /**
   * The run this offer belongs to — one timestamp shared by every offer of one
   * digest run. The day view shows *the latest run of a day*, never the union
   * of a day's runs (measured 2026-08-20: a 20:51 run and a 23:00 UTC run on
   * the same date displayed as one eight-entry digest — a broken ceiling).
   * Optional because pre-2026-08-20 offers lack it; readers fall back to `at`,
   * which those runs wrote identically across each run's offers.
   */
  runAt?: string;
