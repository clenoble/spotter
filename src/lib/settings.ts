...
export interface BackendSettings {
  ...
  searxngUrl?: string;
  /**
   * Candidates examined per search funnel (fetch + all axes each). The cost
   * envelope, owned by the user since 2026-08-21 — it was a constant (20) set
   * during the autonomous build, which made a third of a real haul silently
   * ungoverned by anyone. Run duration grows roughly linearly with it.
   */
  fetchBudget?: number;
  /** The companion (host #3). URL probed on wake; empty token = not paired. */
  companionUrl?: string;
